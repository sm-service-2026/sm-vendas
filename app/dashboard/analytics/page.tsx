          ...
          <div className="flex justify-between h-16 items-center max-w-7xl mx-auto">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h1 className="text-2xl font-bold" style={{ color: theme.text }}>📊 Analytics</h1>
                  <span className="px-3 py-1 text-sm font-medium rounded-full" style={{ backgroundColor: 'rgba(93, 173, 226, 0.2)', color: theme.indigo, border: `1px solid rgba(93, 173, 226, 0.3)` }}>
                    {ano}
                  </span>
                </div>
                <p className="text-sm mt-1" style={{ color: theme.textSecondary }}>
                  Visualização avançada de dados e insights estratégicos
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2 rounded-lg p-1" style={{ backgroundColor: 'rgba(0, 0, 0, 0.2)' }}>
                <button
                  onClick={() => setTipoVisao('vendas')}
                  className="px-3 py-1.5 text-sm font-medium rounded-md transition-colors"
                  style={{ 
                    backgroundColor: tipoVisao === 'vendas' ? theme.indigo : 'transparent',
                    color: tipoVisao === 'vendas' ? 'white' : theme.textSecondary
                  }}
                >
                  💰 Vendas
                </button>
                <button
                  onClick={() => setTipoVisao('tecnicos')}
                  className="px-3 py-1.5 text-sm font-medium rounded-md transition-colors"
                  style={{ 
                    backgroundColor: tipoVisao === 'tecnicos' ? theme.orange : 'transparent',
                    color: tipoVisao === 'tecnicos' ? 'white' : theme.textSecondary
                  }}
                >
                  👨‍🔧 Técnicos
                </button>
                <button
                  onClick={() => setTipoVisao('tecnologia')}
                  className="px-3 py-1.5 text-sm font-medium rounded-md transition-colors"
                  style={{ 
                    backgroundColor: tipoVisao === 'tecnologia' ? theme.purple : 'transparent',
                    color: tipoVisao === 'tecnologia' ? 'white' : theme.textSecondary
                  }}
                >
                  💻 Tecnologia
                </button>
                <button
                  onClick={() => setTipoVisao('logistica')}
                  className="px-3 py-1.5 text-sm font-medium rounded-md transition-colors"
                  style={{ 
                    backgroundColor: tipoVisao === 'logistica' ? theme.warning : 'transparent',
                    color: tipoVisao === 'logistica' ? '#2d2d2d' : theme.textSecondary
                  }}
                >
                  🚛 Logística
                </button>
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="rounded-xl p-6" style={{ backgroundColor: 'rgba(39, 174, 96, 0.1)', border: `1px solid ${CORES.green}30` }}>
                    ...
                    <p className="text-3xl font-bold" style={{ color: theme.text }}>
                      {dadosAnalytics.kpis.totalKM.toLocaleString('pt-BR')} km
                    </p>
                    <p className="text-xs mt-2" style={{ color: theme.textMuted }}>
                      Média mensal: {(dadosAnalytics.kpis.totalKM / 12).toLocaleString('pt-BR')} km
                    </p>
                  </div>
